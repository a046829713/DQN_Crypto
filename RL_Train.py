...
class RL_Train():
    def __init__(self) -> None:
        ...
        # 訓練環境
        data = DataFeature().get_train_net_work_data(
            data_path=r'DQN\simulation\data\BTCUSDT-F-30-Min.csv')
        ...
        state = State1D(bars_count=self.BARS_COUNT,
                        ...
                        reward_on_close=self.REWARD_ON_CLOSE
                        )
        # 製作環境
    ...
    def load_pre_train_model_state(self):
        ...
        if checkpoint_path and os.path.isfile(checkpoint_path):
            ...
            self.saves_path = os.path.join(self.SAVES_PATH, datetime.strftime(
                datetime.now(), "%Y%m%d-%H%M%S") + '-' + str(self.BARS_COUNT) + 'k-' + str(self.REWARD_ON_CLOSE) + '-' + self.symbol_code)
